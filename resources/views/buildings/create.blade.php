...
@section('title', 'Create New Building - Utility Wise')
...
<div class="dashboard-wrapper">
    <div class="page-header">
        <div>
            <h1 class="page-title">Create New Building</h1>
            <p class="page-subtitle">Fill in the details below to add a new building to your portfolio</p>
        </div>
    </div>

    <!-- Form Container -->
    <div class="form-container">
        <form action="{{ route('buildings.store') }}" method="POST" id="createForm">
            @csrf
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="building_type" class="form-control @error('building_type') error @enderror" required>
                            ...
                            <option value="mixed" {{ old('building_type') == 'mixed' ? 'selected' : '' }}>Mixed-Use</option>
                        </select>
                        @error('building_type')
                            <div class="error-message">{{ $message }}</div>
                        @enderror
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-control form-textarea @error('description') error @enderror" 
                              placeholder="Brief description of the building, features, amenities...">{{ old('description') }}</textarea>
                    @error('description')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
            ...
            form.addEventListener('submit', function(e) {
                ...
                requiredFields.forEach(field => {
                    if (!field.value.trim()) {
                        ...
                        let errorMsg = field.parentNode.querySelector('.error-message:not(.server-error)');
                        if (!errorMsg) {
